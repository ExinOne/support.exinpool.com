...
ExinPool 是一个在 Mixin 生态为用户提供流动性的一站式解决方案。目前支持的公链有 Mixin Network（XIN）、Ethereum（ETH）、Horizen（ZEN）、Avalanche（AVAX）、Axie Infinity（AXS）、Solana（SOL）、NEAR Protocol（NEAR）、Cosmos（ATOM）、Polkadot（DOT）、Tron（TRX）、Akash（AKT）、Polygon（MATIC）。
